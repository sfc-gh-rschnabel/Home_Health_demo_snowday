"""
Home Health Operations Analytics Dashboard - Streamlit in Snowflake (SiS)
...
st.set_page_config(
    page_title="Home Health Operations Dashboard",
    page_icon="🫁",
    layout="wide",
...
# Sidebar
st.sidebar.markdown("## 🫁 Home Health Operations")
...
if tab_selection == "Executive Summary":
    st.markdown('<h1 class="main-header">Home Health Operations Command Center</h1>', unsafe_allow_html=True)
    ...
    claims = run_query("SELECT COUNT(*) as total, SUM(billed_amount) as billed, SUM(CASE WHEN claim_status='DENIED' THEN billed_amount ELSE 0 END) as at_risk, ROUND(COUNT(CASE WHEN claim_status='DENIED' THEN 1 END)*100.0/COUNT(*),1) as denial_rate FROM HOME_HEALTH_DEMO.RAW_DATA.CLAIMS_SUBMISSIONS")
    ...
    with col1:
        ...
        denials = run_query("SELECT root_cause, COUNT(*) as cnt, SUM(denied_amount) as amt FROM HOME_HEALTH_DEMO.RAW_DATA.CLAIMS_DENIALS GROUP BY root_cause ORDER BY cnt DESC")
    ...
    with col2:
        ...
        refs = run_query("SELECT equipment_category, COUNT(*) as cnt, SUM(revenue) as rev FROM HOME_HEALTH_DEMO.RAW_DATA.PHYSICIAN_REFERRALS GROUP BY equipment_category ORDER BY rev DESC LIMIT 5")
        if not refs.empty:
    ...
    with col3:
        ...
        calls = run_query("SELECT phone_system, COUNT(*) as total, ROUND(COUNT(CASE WHEN abandoned THEN 1 END)*100.0/COUNT(*),1) as abandon_rate, ROUND(AVG(handle_time_seconds),0) as aht FROM HOME_HEALTH_DEMO.RAW_DATA.CALL_DETAIL_RECORDS GROUP BY phone_system")
        if not calls.empty:
...
# ============================================================================
    ...
    if not kpis.empty:
        ...
        recovery = run_query("SELECT ROUND(SUM(recovered_amount)*100.0/NULLIF(SUM(CASE WHEN outcome IS NOT NULL THEN recovered_amount END + (SELECT SUM(denied_amount) FROM HOME_HEALTH_DEMO.RAW_DATA.CLAIMS_DENIALS)),0),1) as rate, SUM(recovered_amount) as recovered FROM HOME_HEALTH_DEMO.RAW_DATA.DENIAL_APPEALS")
        if not recovery.empty:
            with col5: metric_card("Recovered", f"${recovery['RECOVERED'].iloc[0]:,.0f}", "Via appeals", "🔄")

    col1, col2 = st.columns(2)
    with col1:
        trend = run_query("SELECT DATE_TRUNC('week', submission_date)::DATE as week, ROUND(COUNT(CASE WHEN claim_status='DENIED' THEN 1 END)*100.0/COUNT(*),2) as rate FROM HOME_HEALTH_DEMO.RAW_DATA.CLAIMS_SUBMISSIONS GROUP BY week ORDER BY week")
        if not trend.empty:
    ...
    with col2:
        payer = run_query("SELECT payer_name, COUNT(*) as total, COUNT(CASE WHEN claim_status='DENIED' THEN 1 END) as denied, ROUND(COUNT(CASE WHEN claim_status='DENIED' THEN 1 END)*100.0/COUNT(*),1) as rate FROM HOME_HEALTH_DEMO.RAW_DATA.CLAIMS_SUBMISSIONS GROUP BY payer_name ORDER BY rate DESC")
        if not payer.empty:
    ...
    col1, col2 = st.columns(2)
    with col1:
        root = run_query("SELECT root_cause, denial_code, denial_reason, COUNT(*) as cnt, SUM(denied_amount) as amt FROM HOME_HEALTH_DEMO.RAW_DATA.CLAIMS_DENIALS GROUP BY root_cause, denial_code, denial_reason ORDER BY cnt DESC LIMIT 10")
        if not root.empty:
            st.markdown("### Root Cause Analysis")
            st.dataframe(root, use_container_width=True)

    with col2:
        equip = run_query("SELECT equipment_category, COUNT(*) as denials, SUM(denied_amount) as amount FROM HOME_HEALTH_DEMO.RAW_DATA.CLAIMS_DENIALS GROUP BY equipment_category ORDER BY denials DESC")
        if not equip.empty:
...
# ============================================================================
    ...
    """)
    ref_rev = run_query("SELECT SUM(revenue) as total_rev, COUNT(DISTINCT physician_npi) as physicians FROM HOME_HEALTH_DEMO.RAW_DATA.PHYSICIAN_REFERRALS")
    ...
    col1, col2 = st.columns(2)
    with col1:
        reps = run_query("SELECT rep_name, territory, COUNT(CASE WHEN referral_generated THEN 1 END) as referrals, COUNT(*) as activities, ROUND(COUNT(CASE WHEN referral_generated THEN 1 END)*100.0/NULLIF(COUNT(*),0),1) as conv FROM HOME_HEALTH_DEMO.RAW_DATA.SALES_REP_ACTIVITY GROUP BY rep_name, territory ORDER BY referrals DESC LIMIT 15")
        if not reps.empty:
    ...
    with col2:
        market = run_query("SELECT state, SUM(total_claims) as market, SUM(CASE WHEN home_health_share THEN total_claims ELSE 0 END) as home_health, ROUND(SUM(CASE WHEN home_health_share THEN total_claims ELSE 0 END)*100.0/NULLIF(SUM(total_claims),0),1) as share FROM HOME_HEALTH_DEMO.RAW_DATA.CMS_RESPIRATORY_CLAIMS GROUP BY state ORDER BY share DESC LIMIT 15")
        if not market.empty:
    ...
    spec = run_query("SELECT physician_specialty, COUNT(*) as referrals, SUM(revenue) as revenue FROM HOME_HEALTH_DEMO.RAW_DATA.PHYSICIAN_REFERRALS GROUP BY physician_specialty ORDER BY referrals DESC")
    if not spec.empty:
        col1, col2 = st.columns(2)
        with col1:
...
# ============================================================================
    ...
    col1, col2 = st.columns(2)
    with col1:
        systems = run_query("SELECT phone_system, COUNT(*) as calls, ROUND(AVG(handle_time_seconds),0) as aht, ROUND(COUNT(CASE WHEN abandoned THEN 1 END)*100.0/COUNT(*),1) as abandon, ROUND(COUNT(CASE WHEN first_call_resolution THEN 1 END)*100.0/NULLIF(COUNT(CASE WHEN NOT abandoned THEN 1 END),0),1) as fcr, ROUND(AVG(wait_time_seconds),0) as asa FROM HOME_HEALTH_DEMO.RAW_DATA.CALL_DETAIL_RECORDS GROUP BY phone_system")
        if not systems.empty:
            st.markdown("### Phone System Comparison")
            st.dataframe(systems, use_container_width=True)

    with col2:
        types = run_query("SELECT call_type, COUNT(*) as calls, ROUND(AVG(handle_time_seconds)/60.0,1) as aht_min, ROUND(COUNT(CASE WHEN abandoned THEN 1 END)*100.0/COUNT(*),1) as abandon FROM HOME_HEALTH_DEMO.RAW_DATA.CALL_DETAIL_RECORDS WHERE direction='Inbound' GROUP BY call_type ORDER BY calls DESC")
        if not types.empty:
    ...
    col1, col2 = st.columns(2)
    with col1:
        weekly = run_query("SELECT DATE_TRUNC('week', call_date)::DATE as week, phone_system, COUNT(*) as calls FROM HOME_HEALTH_DEMO.RAW_DATA.CALL_DETAIL_RECORDS GROUP BY week, phone_system ORDER BY week")
        if not weekly.empty:
            fig = px.line(weekly, x='WEEK', y='CALLS', color='PHONE_SYSTEM', title="Weekly Call Volume by System", markers=True)
            st.plotly_chart(fig, use_container_width=True)

    with col2:
        sat = run_query("SELECT interaction_type, ROUND(AVG(overall_score),1) as score, ROUND(COUNT(CASE WHEN would_recommend THEN 1 END)*100.0/COUNT(*),0) as recommend FROM HOME_HEALTH_DEMO.RAW_DATA.PATIENT_SATISFACTION GROUP BY interaction_type ORDER BY score DESC")
        if not sat.empty:
...
# ============================================================================
    ...
    st.markdown('<h1 class="main-header">Home Health AI Operations Assistant</h1>', unsafe_allow_html=True)
    st.markdown("""
    ...
    st.markdown("---")
    st.markdown("### Chat with Home Health Operations Agent")
    st.markdown("Access the full agent experience at: **AI & ML > Snowflake Intelligence** in Snowsight")
    st.markdown("Agent: `SNOWFLAKE_INTELLIGENCE.AGENTS.HOME_HEALTH_OPERATIONS_AGENT`")

    user_question = st.text_input("Ask a question about Home Health operations:", placeholder="e.g., What is our denial rate for Medicare CPAP claims?")
    if user_question:
        ...
        if "denial" in user_question.lower():
            result = run_query("SELECT payer_name, ROUND(COUNT(CASE WHEN claim_status='DENIED' THEN 1 END)*100.0/COUNT(*),2) as denial_rate, COUNT(*) as total_claims FROM HOME_HEALTH_DEMO.RAW_DATA.CLAIMS_SUBMISSIONS GROUP BY payer_name ORDER BY denial_rate DESC")
            if not result.empty:
            ...
            result = run_query("SELECT rep_name, territory, COUNT(CASE WHEN referral_generated THEN 1 END) as referrals, ROUND(COUNT(CASE WHEN referral_generated THEN 1 END)*100.0/COUNT(*),1) as conversion_rate FROM HOME_HEALTH_DEMO.RAW_DATA.SALES_REP_ACTIVITY GROUP BY rep_name, territory ORDER BY referrals DESC LIMIT 10")
            if not result.empty:
                st.dataframe(result, use_container_width=True)
...
# Footer
st.markdown("---")
st.caption("Home Health Operations Dashboard | Powered by Snowflake | Dynamic Tables + Semantic View + Cortex Search + Intelligence Agent")
